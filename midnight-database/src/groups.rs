...
use futures::future;
use midnight_model::{
    database::{OsuGroup, OsuUserGroupGamemodes, OsuUserGroups, OsuUsers},
    osu::{GamemodeUpdate, OsuUser},
    schema::{
...
pub async fn update_group_member_gamemodes(
    conn: &mut PooledConn<'_>,
    user_id: i32,
    diff: &GamemodeUpdate,
) -> Result<()> {
...
mod tests {
    ...
    use diesel_async::RunQueryDsl;
    use midnight_model::{database::OsuGamemode, schema};
    ...
    async fn init_db() {
        DB.get_or_init(|| async {
            Database::new("postgres://postgres@127.0.0.1:5432/testing")
                .await
                .unwrap()
        })
        .await;
    ...
    async fn get_conn() -> PooledConn<'static> {
        DB.get().unwrap().get_conn().await
    }
    ...
    async fn fetch_per_group() {
        ...
        insert_group_member(conn, &user1, OsuGroup::BeatmapNominator)
            .await
            .unwrap();
        insert_group_member(conn, &user1, OsuGroup::ProjectLoved)
            .await
            .unwrap();

        insert_group_member(conn, &user2, OsuGroup::BeatmapNominator)
            .await
            .unwrap();

        let bn_res = fetch_all_group_members(conn, OsuGroup::BeatmapNominator)
            .await
            .unwrap();
        let loved_res = fetch_all_group_members(conn, OsuGroup::ProjectLoved)
            .await
            .unwrap();
        ...
        delete_group_member(conn, 727, OsuGroup::BeatmapNominator)
            .await
            .unwrap();
        ...
        delete_group_member(conn, 727, OsuGroup::ProjectLoved)
            .await
            .unwrap();
        delete_group_member(conn, 728, OsuGroup::BeatmapNominator)
            .await
            .unwrap();
        bn_expected.drain();
    ...
    async fn update_gamemodes() {
        ...
        insert_group_member(conn, &user, OsuGroup::BeatmapNominator)
            .await
            .unwrap();
        ...
        let res = fetch_all_group_members(conn, OsuGroup::BeatmapNominator)
            .await
            .unwrap();
        delete_group_member(conn, 729, OsuGroup::BeatmapNominator)
            .await
            .unwrap();
    ...
    async fn update_user_profile() {
        ...
        insert_group_member(conn, &old, OsuGroup::BeatmapNominator)
            .await
            .unwrap();
        ...
        delete_group_member(conn, 1000, OsuGroup::BeatmapNominator)
            .await
            .unwrap();
