...
use midnight_model::{
    database::{BeatmapsetSubscriptions, Beatmapsets},
    schema::{
...
pub async fn insert_beatmaps(conn: &mut PooledConn<'_>, ids: Vec<i32>) -> Result<()> {
    let ids = ids
        .iter()
...
pub async fn delete_beatmap(conn: &mut PooledConn<'_>, beatmapset_id: i32) -> Result<()> {
...
pub async fn fetch_all_tracked(conn: &mut PooledConn<'_>) -> Result<Option<Vec<i32>>> {
    let rows = beatmapsets.load::<Beatmapsets>(conn).await?;
...
pub async fn fetch_all_subscribers_for_beatmap(
...
pub async fn fetch_all_subscriptions_for_user(
    conn: &mut PooledConn<'_>,
    user_id: i64,
) -> Result<Option<SmallVec<[i32; 8]>>> {
