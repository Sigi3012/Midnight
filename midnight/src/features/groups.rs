...
use midnight_database::groups::{
    delete_group_member, fetch_all_group_members, insert_group_member,
...
use midnight_model::{
    database::OsuGroup,
    osu::{Diff, GamemodeUpdate, OsuUser},
};
...
impl GroupManager {
    pub fn new() -> Self {
        ...
        task::spawn(async move {
            for group in TRACKED_OSU_GROUPS {
                task::spawn(async move {
                    if let Err(why) = update_group(group).await {
                        error!("Failed to update {:?}: {:?}", group, why);
                        time::sleep(ERROR_BACKOFF_COOLDOWN).await;
                    }
                });
            }
            time::sleep(GROUP_LOOP_DURATION).await;
        });
...
async fn update_group(group: OsuGroup) -> Result<()> {
    ...
    for user in diff.added {
        insert_group_member(&mut conn, &user, group).await?
    }
    for user in diff.removed {
        delete_group_member(&mut conn, user.id, group).await?
    }
    for user in diff.updated {
        update_group_member_gamemodes(&mut conn, user.0.id, &user.1).await?
    }
...
mod test {
    use super::*;
    use midnight_model::{database::OsuGamemode, osu::OsuUser};
